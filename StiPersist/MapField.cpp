...
namespace StiPersist
{
	namespace Data
	{
		...
		Data::Chunk* MapField::getDataChunk(void)
		{
			...
			{
				Data::Chunk *keyChunk = Data::Chunk::FromString(iterator->getKey());
				
				Data::Buffer *ibuffer = iterator->getElement()->getChunkBuffer();
				
				Data::Chunk *dataChunk = ibuffer->getChunk();
		...
		void MapField::fromDataChunk(Data::Chunk *dataChunk)
		{
			...
			while(current < length)
			{
				...
				Data::Chunk *keyChunk = new Chunk(k_data, marker->keyLength);
				Data::Chunk *dataChunk = new Chunk(f_data, marker->length);
